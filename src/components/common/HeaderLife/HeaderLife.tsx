...
function validationPage(windowWidthPage: any, setIsRegionOpen: any) {
  if (
    ...
    if (
      !localStorage.getItem('onSiteLife') ||
...
const getInfo = async (
  ...
  if (!!regionData?.city && !!regionData?.state) {
    ...
    validationPage(windowWidthPage, isPageSearch)
  }
}
...
const HeaderLife: FC<Props> = ({ isPDP, location, content }) => {
  ...
  const menuItemDesktop = JSON.parse(categoriesMenu?.[0]?.data?.MenuDesktopLife)
  const menuItemMobile = JSON.parse(categoriesMenu?.[0]?.data?.MenuMobileLife)
  ...
  useEffect(() => {
    ...
    if (window.location.pathname === '/categoria/life') {
      setTimeout(() => {
        localStorage.setItem('onSiteLife', 'Is on site')
      }, 1000)
    }
  ...
  return (
    <>
      ...
      >
        ...
        <div data-lifeheader-wrapper style={headerTopRowSlideUp}>
          <div className="lifeheader-content-wrapper">
            ...
            <span title="Life by Vivara" data-life-title>
              <LifeLogo />
            </span>
            <div data-vivara-wrapper>
              <p data-vivara-text>Visite também</p>
              <Logo />
            </div>
            <button
              data-region-wrapper
